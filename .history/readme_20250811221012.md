...
- cards have fixed width - `$card-width`
- the gap between cards should be - `46px` horizontally and `48px` vertically
- cards container(catalog) have fixed paddings (`$space-50` vertically and `$space-40` horizontally)

Make all the changes smooth on hover (during 300ms):
- increase the card by 20 percent (neighboring cards **should not be** affected)
- change the card title text color to `$color-accent` when the card is hovered (`.card:hover .card__title`)
